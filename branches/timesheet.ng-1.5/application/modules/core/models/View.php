...
class Core_View {
	...
	{
		return new Core_View($name, $data, $type);
	}
	...
	public function set_filename($name, $type = NULL)
	{
		if ($type == NULL)
		{
			...
			$this->kohana_filename = $this->get_view_filename($name).EXT;
			$this->kohana_filetype = EXT;
		}
		else
		{
			// Check if the filetype is allowed by the configuration
			if ( ! in_array($type, TSNG::config('view.allowed_filetypes')))
				throw new Core_Exception('core.invalid_filetype', $type);

			// Load the filename and set the content type
			$this->kohana_filename = $this->get_view_filename($name);
	...
	public function render($print = FALSE, $renderer = FALSE)
	{
		if (empty($this->kohana_filename))
			throw new Core_Exception('core.view_set_filename');

		if (is_string($this->kohana_filetype))
		{
			// Merge global and local data, local overrides global with the same name
			$data = array_merge(Core_View::$kohana_global_data, $this->kohana_local_data);

			// Load the view in the controller for access to $this
			$output = TSNG::$instance->_kohana_load_view($this->kohana_filename, $data);
	...
	protected function get_view_filename($name) {
			$parts = explode('.', $name);
			$module = $parts[0];
    		$dir = '';
			if(isset($parts[2])) {
	    	...
	    	return MODULES.$module.DS.'views'.$dir.DS.$file; 
	}
} // End View