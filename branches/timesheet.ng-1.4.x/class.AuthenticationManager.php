...
enum( "AUTH_NONE", // no attempt has been made to authenticate yet
...
enum ( "LDAP_AUTH_NONE", // no LDAP authentication has been attempted
...
class AuthenticationManager{
	...
	*/
	function login( $username, $password ){
		...
		list( $qh, $num ) = dbQuery( "SELECT level " . "FROM $USER_TABLE WHERE username='$username'" );
		$data = dbResult( $qh );
	...
	*/
	...
	function ldapLogin( $username, $password ){
		...
		list( $qh, $num ) = dbQuery( "SELECT LDAPScheme, LDAPHost, LDAPPort, LDAPBaseDN, " . "LDAPUsernameAttribute, LDAPSearchScope, LDAPFilter, LDAPProtocolVersion, " . "LDAPBindUsername, LDAPBindPassword " . "FROM $CONFIG_TABLE WHERE config_set_id='1'" );
		$data = dbResult( $qh );
		...
		if ( !$this->userExists( $username ) ){
			// create the user
			dbquery( "INSERT INTO $USER_TABLE (username, level, password, first_name, last_name, " . "email_address, phone, bill_rate, time_stamp, status) " . "VALUES ('$username',1,$DATABASE_PASSWORD_FUNCTION('$password'),'$firstName'," . "'$lastName','$emailAddress','$phone','$billRate',0,'OUT')" );
			...
			dbquery( "UPDATE $USER_TABLE SET first_name='$firstName', last_name='$lastName', " . "email_address='$emailAddress', phone='$phone', bill_rate='$billRate' " . "WHERE username='$username'" );
		}
