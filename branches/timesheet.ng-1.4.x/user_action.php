...
if ($action == "delete") {
	...
	if ($data = dbResult($qh)) {
		...
		if ($data["password"] == $password) {
			...
			dbquery("UPDATE $USER_TABLE SET first_name='$first_name', last_name='$last_name', " . "username='$username', " . "email_address='$email_address', phone='$phone', bill_rate='$bill_rate', " . "level='$level' " . "WHERE uid='$uid'");
		}
		else{
			//set the password as well
			dbquery("UPDATE $USER_TABLE SET first_name='$first_name', last_name='$last_name', " . "username='$username', " . "email_address='$email_address', phone='$phone', bill_rate='$bill_rate', " . "level='$level', " . "password=$DATABASE_PASSWORD_FUNCTION('$password') " . "WHERE username='$username'");
		}
	}
	else{
		// a new user
		dbquery("INSERT INTO $USER_TABLE (username, level, password, allowed_realms, first_name, " . "last_name, email_address, phone, bill_rate, time_stamp, status) " . "VALUES ('$username',$level,$DATABASE_PASSWORD_FUNCTION('$password'),'.*','$first_name'," . "'$last_name','$email_address','$phone','$bill_rate',0,'OUT')");
