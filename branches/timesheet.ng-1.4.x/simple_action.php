...
$queryString = "DELETE FROM $TIMES_TABLE WHERE " . "uid='$contextUser' AND " . "start_time >= '$startYear-$startMonth-$startDay 00:00:00' AND " . "start_time < '$endYear-$endMonth-$endDay 00:00:00'";
dbQuery( $queryString );
// TODO:...
for ( $i = 0; $i < $totalRows; $i++ ){
	...
	for ( $j = 1; $j <= 7; $j++ ){
		...
		if ( ( !empty( $hours ) && $hours != 0 ) || ( !empty( $mins ) && $mins != 0 ) ){
			...
			// add to database
			$queryString = "INSERT INTO $TIMES_TABLE (uid, start_time, end_time, proj_id, task_id, log_message) " . "VALUES ('$contextUser','$todayYear-$todayMonth-$todayDay 00:00:00', " . "'$todayYear-$todayMonth-$todayDay $hours:$mins:00', " . "$projectId, $taskId, 'Task time entered via simple.php')";
