...
?>&year=<?php echo $last_year ?>&month=<?php echo $last_month ?>&day=<?php echo $todayDay;
?>" class="outer_table_action">Prev</a>
							<a href="<?php echo $_SERVER["PHP_SELF"];
?>?client_id=<?php echo $client_id;
?>&proj_id=<?php echo $proj_id;
?>&task_id=<?php echo $task_id;
?>&year=<?php echo $next_year ?>&month=<?php echo $next_month ?>&day=<?php echo $todayDay;
?>" class="outer_table_action">Next</a>
						</td>
					</tr>
				</table>
...
<?php
	...
	while (checkdate($month, $day, $year)) {
		...
		for ($i=0;$i<$num; $i++) {
			...
			if (
				(($data["start_time"] < mktime(0, 0, 0, $month, $day, $year)) && ($data["end_time"] > mktime(23, 59, 59, $month, $day, $year))) ||
