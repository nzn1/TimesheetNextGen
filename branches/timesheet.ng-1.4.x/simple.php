...
$query .= "unix_timestamp(end_time) - unix_timestamp(start_time) AS diff_sec, " . "end_time AS end_time_str, " . "start_time AS start_time_str, " . "unix_timestamp(start_time) AS start_time, " . "unix_timestamp(end_time) AS end_time, " . "$PROJECT_TABLE.title AS projectTitle, " . "$TASK_TABLE.name AS taskName, " . "$TIMES_TABLE.proj_id, " . "$TIMES_TABLE.task_id " . "FROM $TIMES_TABLE, $TASK_TABLE, $PROJECT_TABLE WHERE " . "$PROJECT_TABLE.proj_id = $TIMES_TABLE.proj_id AND " . "uid='$contextUser' AND ";

if ($proj_id > 0) // otherwise want all records no matter what project
	$query .= "$TIMES_TABLE.proj_id=$proj_id AND ";

$query .= "$TASK_TABLE.task_id = $TIMES_TABLE.task_id AND " . "((start_time >= '$startYear-$startMonth-$startDay 00:00:00' AND " . "start_time < '$endYear-$endMonth-$endDay 00:00:00') " . "OR (end_time >= '$startYear-$startMonth-$startDay 00:00:00' AND " . "end_time < '$endYear-$endMonth-$endDay 00:00:00') " . "OR (start_time < '$startYear-$startMonth-$startDay 00:00:00' AND end_time >= '$endYear-$endMonth-$endDay 00:00:00')) " . "ORDER BY proj_id, start_time";
...
$getProjectsQuery = "SELECT $PROJECT_TABLE.proj_id, $PROJECT_TABLE.title, $PROJECT_TABLE.client_id, " . "$CLIENT_TABLE.client_id, $CLIENT_TABLE.organisation FROM " . "$PROJECT_TABLE, $ASSIGNMENTS_TABLE, $CLIENT_TABLE where " . "$PROJECT_TABLE.proj_id=$ASSIGNMENTS_TABLE.proj_id AND " . "$ASSIGNMENTS_TABLE.username='$contextUser' AND " . "$PROJECT_TABLE.client_id=$CLIENT_TABLE.client_id " . "ORDER BY $CLIENT_TABLE.organisation, $PROJECT_TABLE.title";
...
$getTasksQuery = "SELECT $TASK_TABLE.proj_id, $TASK_TABLE.task_id, $TASK_TABLE.name from " . "$TASK_TABLE, $TASK_ASSIGNMENTS_TABLE where " . "$TASK_TABLE.task_id = $TASK_ASSIGNMENTS_TABLE.task_id AND " . "$TASK_ASSIGNMENTS_TABLE.username='$contextUser'";
...
?>&year=<?php echo $previousWeekYear ?>&month=<?php echo $previousWeekMonth ?>&day=<?php echo $previousWeekDay ?>" class="outer_table_action">Prev</a>
							<a href="<?php echo $_SERVER["PHP_SELF"];
...
?>&year=<?php echo $nextWeekYear ?>&month=<?php echo $nextWeekMonth ?>&day=<?php echo $nextWeekDay ?>" class="outer_table_action">Next</a>
						</td>
						<td align="right" nowrap>
...
for ($i=0; $i<7; $i++) {
	...
	print
	"<td align=\"center\" width=\"65\">" . "<table width=\"65\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr>" . "<td class=\"inner_table_column_heading\" align=\"center\">" . "$currentDayStr" . "</td></tr></table></td>\n";
}
...
for ($i = 0; $i < $num5; $i++) {
	...
	for ($k = 0; $k < 7; $k++) {
		...
		$startsToday = ($currentTaskStartDate >= ($startDate + $k * A_DAY) && $currentTaskStartDate < ($startDate + ($k + 1) * A_DAY));
		$endsToday = ($currentTaskEndDate >= ($startDate + $k * A_DAY) && $currentTaskEndDate < ($startDate + ($k + 1) * A_DAY));
...
for ($i = 0; $i < $count; $i++) {
	...
	?><input type="hidden" id="project_row<?php echo $i;

	?>" name="project_row<?php echo $i;

	?>" value="<?php echo $matchedPair->projectId;

	?>" /><?php
	?><select id="projectSelect_row<?php echo $i;

	?>" name="projectSelect_row<?php echo $i;

	?>" onChange="onChangeProjectSelect(this.id);" style="width: 100%;" /><?php
	?></td><?php
	?><td width="50%" align="left"><?php
	?><input type="hidden" id="task_row<?php echo $i;

	?>" name="task_row<?php echo $i;

	?>" value="<?php echo $matchedPair->value1;

	?>" /><?php
	?><select id="taskSelect_row<?php echo $i;

	?>" name="taskSelect_row<?php echo $i;

	?>" onChange="onChangeTaskSelect(this.id);" style="width: 100%;" /><?php
	?></td><?php
	?></tr><?php
	?><tr><?php
...
?><tr><?php
?><td width="50%" align="left"><?php
?><input type="hidden" id="project_row<?php echo $count;

?>" name="project_row<?php echo $count;

?>" value="-1" /><?php
?><select id="projectSelect_row<?php echo $count;

?>" name="projectSelect_row<?php echo $count;

?>" onChange="onChangeProjectSelect(this.id);" style="width: 100%;" /><?php
?></td><?php
?><td width="50%" align="left"><?php
?><input type="hidden" id="task_row<?php echo $count;

?>" name="task_row<?php echo $count;

?>" value="-1" /><?php
?><select id="taskSelect_row<?php echo $count;

?>" name="taskSelect_row<?php echo $count;
