...
class UnitTest {
	...
	public function __construct()
	{
		...
		$paths = array_merge(func_get_args(), TSNG::config('unit-test.paths', FALSE, FALSE));
		...
		foreach ($this->paths as $path)
		{
			// Validate test path
			if ( ! is_dir($path))
				throw new Core_Exception('unit-test.invalid_test_path', $path);
			...
			)
			{
				...
				if (class_exists($class, FALSE))
					throw new Core_Exception('unit-test.duplicate_test_class', $class, $path);
				...
				if ( ! class_exists($class, FALSE))
					throw new Core_Exception('unit-test.test_class_not_found', $class, $path);
				...
				if ( ! $reflector->isSubclassOf(new ReflectionClass('Unit_Test_Case')))
					throw new Core_Exception('unit-test.test_class_extends', $class);
	...
	public function report($hide_passed = NULL)
	{
		...
		if (empty($this->results))
			return TSNG::lang('unit-test.no_tests_found');

		// Hide passed tests from the report?
		$hide_passed = (bool) (($hide_passed !== NULL) ? $hide_passed : TSNG::config('unit-test.hide_passed', FALSE, FALSE));

		// Render unit_test report
		return Core_View::factory('unit-test.kohana_unit_test')
...
abstract class Unit_Test_Case {
	...
	{
		if ($value != TRUE) {
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_true', gettype($value), var_export($value, TRUE)), $debug);
		}
		return $this;
	}
	...
	{
		if ($value !== TRUE)
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_true_strict', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ($value != FALSE)
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_false', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ($value !== FALSE)
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_false_strict', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ($expected != $actual)
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_equal', gettype($expected), var_export($expected, TRUE), gettype($actual), var_export($actual, TRUE)), $debug);
	...
	{
		if ($expected == $actual)
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_not_equal', gettype($expected), var_export($expected, TRUE), gettype($actual), var_export($actual, TRUE)), $debug);
	...
	{
		if ($expected !== $actual)
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_same', gettype($expected), var_export($expected, TRUE), gettype($actual), var_export($actual, TRUE)), $debug);
	...
	{
		if ($expected === $actual)
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_not_same', gettype($expected), var_export($expected, TRUE), gettype($actual), var_export($actual, TRUE)), $debug);
	...
	{
		if ( ! is_bool($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_boolean', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if (is_bool($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_not_boolean', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ( ! is_int($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_integer', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if (is_int($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_not_integer', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ( ! is_float($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_float', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if (is_float($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_not_float', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ( ! is_array($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_array', gettype($value), var_export($value, TRUE)), $debug);
	...
	public function assert_array_key($key, $array, $debug = NULL)
	{
		if ( ! array_key_exists($key, $array)) {
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_array_key', gettype($key), var_export($key, TRUE)), $debug);
		}
	...
	public function assert_in_array($value, $array, $debug = NULL)
	{
		if ( ! in_array($value, $array)) {
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_in_array', gettype($value), var_export($value, TRUE)), $debug);
		}
	...
	{
		if (is_array($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_not_array', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ( ! is_object($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_object', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if (is_object($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_not_object', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ($value !== NULL)
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_null', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ($value === NULL)
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_not_null', gettype($value), var_export($value, TRUE)), $debug);
	...
	{
		if ( ! empty($value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_empty', gettype($value), var_export($value, TRUE)), $debug);
	...
	public function assert_pattern($value, $regex, $debug = NULL)
	{
		if ( ! is_string($value) OR ! is_string($regex) OR ! preg_match($regex, $value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_pattern', var_export($value, TRUE), var_export($regex, TRUE)), $debug);
	...
	{
		if ( ! is_string($value) OR ! is_string($regex) OR preg_match($regex, $value))
			throw new UnitTest_Exception(TSNG::lang('unit-test.assert_not_pattern', var_export($value, TRUE), var_export($regex, TRUE)), $debug);
...
class UnitTest_Exception extends Exception {
