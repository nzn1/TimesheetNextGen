 ...
 *
 * $Id: ORM.php 4116 2009-03-24 23:36:22Z jheathco $
 *
 * @package    ORM
 * @author     Kohana Team
...
class Core_ORM {
	...
	public function __construct($id = NULL)
	{
		...
		list($module_name, $this->object_name) = explode('_', strtolower(get_class($this)), 2);
		$this->object_plural = Core_Inflector::plural($this->object_name);
	...
	public function __initialize()
	{
		...
		if (empty($this->table_name))
		{
			...
			if ($this->table_names_plural === TRUE)
			{
				...
				$this->table_name = Core_Inflector::plural($this->table_name);
			}
		}
	...
	public function reload_columns($force = FALSE)
	{
		...
		{
			if (isset(Core_ORM::$column_cache[$this->object_name]))
			{
				// Use cached column information
				$this->table_columns = Core_ORM::$column_cache[$this->object_name];
			}
			else
			{
				// Load table columns
				Core_ORM::$column_cache[$this->object_name] = $this->table_columns = $this->db->list_fields($this->table_name, TRUE);
			}
		}
	 ...
	 * @return  boolean
	 */
	public function has(Core_ORM $model, $any = FALSE)
	{
	 ...
	 * @return  boolean
	 */
	public function add(Core_ORM $model)
	{
		if ($this->has($model))
			return TRUE;
	 ...
	 * @return  boolean
	 */
	public function remove(Core_ORM $model)
	{
		if ( ! $this->has($model))
			return FALSE;
	...
	protected function related_object($object)
	{
		...
		{
			$object = Core_ORM::factory($this->has_one[$object]);
		}
		elseif (isset($this->belongs_to[$object]))
		{
			$object = Core_ORM::factory($this->belongs_to[$object]);
		}
		elseif (in_array($object, $this->has_one) OR in_array($object, $this->belongs_to))
		{
			$object = Core_ORM::factory($object);
		}
		else
		{
			return FALSE;
	...
	protected function load_result($array = FALSE)
	{
		...
		if ($array === TRUE)
		{
			...
			return new Core_ORM_Iterator($this, $result);
		}
	 ...
	 * @return  array
	 */
	protected function load_relations($table, Core_ORM $model)
	{
