...
/**
 * This was file has been copied from the Kohana PHP Framework (v2.3.2) 
 * and hacked to fit in with Timesheet Next Gen
 * Full credit goes to the Kohana Team: http://www.kohanaphp.com/
 * Licence Details: http://kohanaphp.com/license
 * --------------------------------
...
class Database {
	 ...
	 */
	public static function & instance($name = 'database', $config = NULL)
	{
	 ...
	 *
	 * @throws  Core_Database_Exception
	 */
	public function __construct()
	{

		// Load the default group
		$config = TSNG::config('core.database');
		...
		if (is_string($this->config['connection']))
		{
			...
			if (strpos($this->config['connection'], '://') === FALSE) {
				throw new Core_Database_Exception('database.invalid_dsn', $this->config['connection']);
			}
		...
		// Set driver name
		$driver = 'Core_Database_'.ucfirst($this->config['connection']['type']);

		// Initialize the driver
		$this->driver = new $driver($this->config);

		// Validate the driver
		if ( ! ($this->driver instanceof Core_Database_Driver)) {
			throw new Core_Database_Exception('core.driver_implements', $this->config['connection']['type'], get_class($this), 'Database_Driver');
		}
	...
	public function connect()
	{
		...
		if ( ! is_resource($this->link) AND ! is_object($this->link))
		{
			...
			if ( ! is_resource($this->link) AND ! is_object($this->link))
				throw new Core_Database_Exception('core.db_connection', $this->driver->show_error());
	...
	public function insert($table = '', $set = NULL)
	{
		...
		if ($this->set == NULL)
			throw new Core_Database_Exception('core.db_must_use_set');

		if ($table == '')
		{
			if ( ! isset($this->from[0]))
				throw new Core_Database_Exception('core.db_must_use_table');
	...
	public function merge($table = '', $set = NULL)
	{
		...
		if ($this->set == NULL)
			throw new Core_Database_Exception('database.must_use_set');
	...
	public function delete($table = '', $where = NULL)
	{
		...
		if (count($this->where) < 1)
			throw new Core_Database_Exception('core.db_must_use_where');
