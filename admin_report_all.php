...
<?
	...
	if ($num == 0) {
		...
		while ($name_data = dbResult($qh)) {
			array_push($dati_total,$name_data);
			...
			array_push($dati_total_time_data,$time_data);
			print "<tr>\n";
	...
	if (!empty($dati_total)) {
		...
		$_SESSION['excel_data_time_data']=$dati_total_time_data;
		$out_js.="window.open('./export_timesheet.php?type=all')";
	}else {
