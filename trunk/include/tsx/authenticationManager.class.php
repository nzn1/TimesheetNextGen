...
//		@session_start();
	...
	*/
	...
	function ldapLogin($username, $password) {
		...
		$connectionString = Site::Config()->get('LDAPurl');
		...
		@ldap_set_option($connection, LDAP_OPT_PROTOCOL_VERSION, Site::Config()->get("LDAPProtocolVersion"));

		// bind to server by user
		if (Site::Config()->get('LDAPBindByUser') == 1) {
			if (Site::Config()->get("LDAPBindUsername") == '') {
				//bind using user supplied info, if there is no BindUsername in the config
				$credentials=Site::Config()->get('LDAPUsernameAttribute') . "=" . $username . "," . Site::Config()->get('LDAPBaseDN');
				...
				if (!($bind = @ldap_bind($connection, Site::Config()->get("LDAPBindUsername"), Site::Config()->get("LDAPBindPassword")))) {
					$this->ldapErrorCode = LDAP_SERVER_ERROR;
					$this->ldapServerErrorCode = ldap_errno($connection);
					$this->ldapServerErrorText = "LDAP: " . ldap_error($connection);
					return false;
		...
		$filter = Site::Config()->get('LDAPUsernameAttribute') . "=" . $username;
		if (Site::Config()->get('LDAPFilter') != "") {
			//does it start with a '(' and end with a ')' ?
			$userFilter = Site::Config()->get("LDAPFilter");
		...
		if ( Site::Config()->get("LDAPReferrals") != 1) {
			@ldap_set_option( $connection, LDAP_OPT_REFERRALS, 0 );
		}

		if (Site::Config()->get("LDAPSearchScope") == "base") {
			//search the directory returning records in the base dn
			//echo "<p>searching base dn: Site::Config()->get(LDAPBaseDN]        with filter: $filter <p>";
			//LogFile::write("searching base ".Site::Config()->get('LDAPBaseDN')." for $filter\n");
			if (!($search = @ldap_read($connection, Site::Config()->get('LDAPBaseDN'), $filter))) {
				$this->ldapErrorCode = LDAP_SERVER_ERROR;
				$this->ldapServerErrorCode = ldap_errno($connection);
				$this->ldapServerErrorText = "LDAP: " . ldap_error($connection);
				return false;
			}
		} else if (Site::Config()->get("LDAPSearchScope") == "one") {
			//search the directory returning records in the base dn
			//echo "<p>searching base dn: Site::Config()->get(LDAPBaseDN]        with filter: $filter <p>";
			//LogFile::write("searching one ".Site::Config()->get('LDAPBaseDN')." for $filter\n");
			if (!($search = @ldap_list($connection, Site::Config()->get('LDAPBaseDN'), $filter))) {
				$this->ldapErrorCode = LDAP_SERVER_ERROR;
				$this->ldapServerErrorCode = ldap_errno($connection);
				$this->ldapServerErrorText = "LDAP: " . ldap_error($connection);
				return false;
			...
			//search the directory returning records in the base dn
			//echo "<p>searching base dn: Site::Config()->get(LDAPBaseDN]        with filter: $filter <p>";
			//LogFile::write("searching sub ".Site::Config()->get('LDAPBaseDN')." for $filter\n");
			if (!($search = @ldap_search($connection, Site::Config()->get('LDAPBaseDN'), $filter))) {
				$this->ldapErrorCode = LDAP_SERVER_ERROR;
				$this->ldapServerErrorCode = ldap_errno($connection);
				$this->ldapServerErrorText = "LDAP: " . ldap_error($connection);
				return false;
