...
class ConfigFactory
{
	...
	public static function initialise() {
		...
		if(!file_exists('install/database_credentials.inc')){
			trigger_error('database_credentials.inc could not be found');
			return;
		}

		require("install/database_credentials.inc");
		self::$version = $TIMESHEET_VERSION;
		self::$isInstalled = $TIMESHEET_INSTALLED;
		self::$dbServer = $DATABASE_HOST;
		self::$dbUser = $DATABASE_USER;
		self::$dbPass = $DATABASE_PASS;
		self::$dbName = $DATABASE_DB;
		self::$dbPasswordFunction = $DATABASE_PASSWORD_FUNCTION;
